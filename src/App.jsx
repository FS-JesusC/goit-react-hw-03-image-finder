    ...
    try {
      const { totalHits, hits } = await API.getImages(API.searchParams);
        if(totalHits || hits.length){ 
          if (page === 1) {toast.success(`🛎️ We found ${totalHits} images.`);};
          if (page >= 1) {
          ...
          if(hits.length < 12){toast.info(`🛎️ No more images for ${query}`);};
        }
        else {
          this.setState ({
          totalHits: null,
          ...
          toast.error("🛎️ Sorry, there are no images matching your search query. Please try again.");
        };
    } catch (error) {
      this.setState({
      totalHits: null,
...
handleFormSearch = (query) => {
  if(query === '') {
    ...
    toast('🛎️ There is nothing to search!');
   };
  this.setState({ 
  query,
  page: 1,
